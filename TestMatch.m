...
distance_threshold = 1; % 距离阈值（米）

R = 6378137; % 地球半径 (米)

%% ================= [修改 1: 新增特性参数] =================
% 基于数据分析：AIS采样间隔中位数11s。
% 设置时间同步阈值为30s (约3倍AIS间隔，容忍一定的时间不对齐)
time_sync_threshold = 30; 

% 基于数据分析：雷达和AIS轨迹平均点数都 >200。
% 为了排除偶然的单点重合，设置最小匹配点数为 5。
min_match_points_threshold = 5; 
%% ========================================================
...
for i = 1:num_UID
    ...
    for j = 1:length(cand_mmsis)
        ...
        for k = 1:length(r_lat)
            ...
            % ================= [修改 3: 增加时间同步约束] =================
            % 找到空间距离最小的点及其索引
            [min_d, idx_min] = min(dists);
            
            % 计算该最近点的时间差 (秒)
            time_diff = abs(seconds(r_time(k) - m_time(idx_min)));
            
            % 判定条件升级：不仅距离要近，时间差也必须在阈值(30s)内
            if min_d <= distance_threshold && time_diff <= time_sync_threshold
        ...
        % 评价匹配质量
        % ================= [修改 4: 增加最小匹配点数限制] =================
        % 只有当匹配点数超过阈值(5个)时，才认为这是一次有效匹配，否则视为噪声
        if num_matches >= min_match_points_threshold
